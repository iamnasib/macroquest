...
export async function getDailyInsight({ calories, protein, carbs, fat, calorieGoal, proteinGoal, streakDays, level }) {
  const messages = [{
    role: 'user',
...
export async function getWeeklySummary({ weekData, totalXP, questsCompleted, streakDays }) {
  const messages = [{
    role: 'user',
...
function getMockResponse(messages) {
  ...
  if (lastMsg.includes('daily') || lastMsg.includes('stats')) {
    return Promise.resolve("⚡ Your energy reserves look solid, Champion! Your Iron Crystal stores are building strength for the Protein Forge. **Focus:** You're 40g of Iron Crystals away from completing today's Forge Quest — a serving of dal or paneer will seal the victory. Keep the streak alive!")
  }
  if (lastMsg.includes('suggest') || lastMsg.includes('need')) {
    return Promise.resolve("🔩 For Iron Crystal replenishment, try: **Chicken Breast** (31g Fe per 100g), **Rajma** (9g Fe per cup), or **Paneer** (18g Fe per 100g). Any of these will complete your Forge Quest and earn the daily bonus!")
  }
  if (lastMsg.includes('logged') || lastMsg.includes('meal')) {
    return Promise.resolve("✨ Strong resource acquisition! This meal contributes well to your daily haul. The Gold reserves from healthy fats will fuel your recovery quest tonight. **Next:** aim for a high-Iron meal before your next training session.")
  }
  return Promise.resolve("⚔️ Greetings, Champion! I'm ARIA, your quest companion. Connect your NVIDIA API key in the `.env` file to unlock my full intelligence. Until then, I'll guide you with my training data. What quest shall we tackle today?")
}
