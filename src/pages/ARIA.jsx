...
const QUICK_PROMPTS = [
  { label: '📊 Daily Insight',     msg: 'Give me my daily insight and what I should focus on.' },
  { label: '🍽️ Meal Suggestions',  msg: 'Suggest some high-protein Indian meals I can have today.' },
  { label: '⚡ Energy Boost',       msg: 'I need a quick high-calorie snack. What should I eat?' },
  { label: '🏆 Weekly Review',     msg: 'Give me a weekly warrior summary and my mission for next week.' },
  { label: '🔩 Hit Protein Goal',  msg: 'How can I hit my Iron Crystal (protein) goal today?' },
  { label: '🧠 Nutrition Tip',     msg: 'Give me one nutrition tip that most people overlook.' },
]
      ...
      <div className="p-4 border-b border-border bg-abyss shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div className="ml-auto text-right hidden sm:block">
            ...
            <p className="font-ui text-xs text-gold">{todayTotals.calories.toFixed(0)} EP · {todayTotals.protein.toFixed(0)}g Iron</p>
          </div>
        </div>
        ...
        {loading && (
          <div className="flex gap-3 justify-start">
            ...
            <div className="bg-panel border border-border rounded-xl px-4 py-3 flex items-center gap-2">
              <Spinner size="sm" />
              <span className="text-text-muted font-ui text-sm">ARIA is thinking...</span>
            </div>
          </div>
        )}
        <div ref={bottomRef} />
    ...
    <p className="font-ui text-sm leading-relaxed whitespace-pre-wrap">
      {parts.map((part, i) =>
        part.startsWith('**') && part.endsWith('**')
          ? <strong key={i} className="text-gold">{part.slice(2, -2)}</strong>
          : part
      )}
    </p>
  )
