...
export default function Character() {
  ...
  const [selectedMode, setSelectedMode] = useState(profile?.game_mode || 'EMPIRE')
  ...
  const nextLevels = [1, 2, 3].map(i => {
    ...
    return { level: l, title: getLevelTitle(l), xpNeeded: getLevelFromXP(totalXP + i * 200).xpNeeded }
  })
  ...
  const handleSave = async () => {
    ...
    try {
      await Promise.all([
        supabase.from('profiles').update({ game_mode: selectedMode }).eq('id', user.id),
        supabase.from('characters').update({ character_type: selectedChar }).eq('user_id', user.id),
      ])
  ...
  return (
    <div className="p-4 lg:p-6 max-w-4xl mx-auto space-y-6">
      <div>
        <h2 className="font-pixel text-gold glow-text-gold" style={{ fontSize: '0.75rem' }}>🐉 CHARACTER</h2>
        <p className="text-text-muted font-ui text-sm mt-1">Your hero, your journey</p>
      </div>
      ...
      <div className="panel pixel-border-gold p-6">
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
          ...
          <div className="grid grid-cols-2 gap-2 sm:w-40">
            <MiniStat label="Total XP"   value={totalXP}                  icon="⭐" color="text-violet" />
            <MiniStat label="Log Streak" value={`${streakData?.logging || 0}d`}  icon="🔥" color="text-rose"   />
            <MiniStat label="Pro Streak" value={`${streakData?.protein || 0}d`}  icon="🔩" color="text-iron"   />
            <MiniStat label="Budget"     value={`${streakData?.budget  || 0}d`}  icon="⚡" color="text-gold"   />
          </div>
        </div>
      </div>
      ...
      <div className="panel p-4">
        ...
        <div className="flex gap-3 overflow-x-auto pb-2">
          {WORLD_STAGES.map(stage => {
            ...
            return (
              <div key={stage.name} className={`shrink-0 w-28 text-center p-3 rounded-lg border transition-all ${
                unlocked
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {Object.values(MODES).map(mode => (
            ...
            >
              ...
              <p className="font-ui text-xs text-text-muted mt-1">{mode.desc}</p>
            </button>
          ))}
        </div>
      </div>
