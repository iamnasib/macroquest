    ...
    <div className='p-4 lg:p-6 max-w-4xl mx-auto space-y-6'>
      <div>
        ...
        <p className='text-text-muted font-ui text-sm mt-1'>
          Complete quests to earn XP, resources, and build your empire
        </p>
      </div>
      ...
      <section>
        <SectionHeader
          ...
          sub='Cumulative achievements'
        />
        <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
          {STREAK_MILESTONES.map((m) => (
            <div
              key={m.id}
              className={`quest-card ${streak >= m.days ? "border-gold/40 bg-gold/5" : "border-border"}`}>
              <div className='flex items-start justify-between mb-2'>
                <div className='flex items-center gap-2'>
                  <span className='text-2xl'>{m.icon}</span>
                  <div>
                    <p
                      className={`font-ui font-semibold text-sm ${streak >= m.days ? "text-gold" : "text-text"}`}>
                      {m.title}
                    </p>
                    <Badge variant={streak >= m.days ? "gold" : "default"}>
                      {m.days}-day streak
                    </Badge>
                  </div>
                </div>
              ...
              <ProgressBar
                value={Math.min(streak, m.days)}
                max={m.days}
                color={streak >= m.days ? "emerald" : "gold"}
              />
              ...
              <div className='flex gap-2 mt-2 flex-wrap'>
                {m.rewards.map((r, i) => (
                  <ResourceChip key={i} type={r.type} amount={r.amount} />
                ))}
              </div>
            </div>
          ))}
      ...
      <section>
        ...
        <div
          className={`panel p-6 text-center ${bossUnlocked ? "border-rose/30" : "border-border opacity-60"}`}>
          {bossUnlocked ? (
            <div>
              <div className='text-5xl mb-3 animate-float'>👿</div>
              <h3
                className='font-pixel text-rose mb-2'
                style={{fontSize: "0.75rem"}}>
                THE WEEKEND BOSS
              </h3>
              <p className='text-text-muted font-ui text-sm mb-1'>
                Health: 10,000 / 10,000
              </p>
              <p className='text-text-muted font-ui text-sm mb-4'>
                Deal damage by completing daily quests. Community event — coming
                soon!
              </p>
              <div className='bg-abyss rounded-lg p-3 max-w-sm mx-auto'>
                <p className='text-xs font-ui text-text-muted'>
                  Appears every Friday. Deal damage by:
                </p>
            ...
            <div>
              ...
              <h3
                className='font-pixel text-text-muted mb-2'
                style={{fontSize: "0.75rem"}}>
                LOCKED — REACH LEVEL {BOSS_UNLOCKS_AT_LEVEL}
              </h3>
              <p className='text-text-muted font-ui text-sm'>
                You are Level {levelData.level}. Keep logging and completing
                quests to unlock boss battles.
              </p>
              <ProgressBar
...
function QuestCard({quest}) {
  ...
  const pct =
    quest.total > 0 ? Math.min((quest.progress / quest.total) * 100, 100) : 0;
  const progressText =
    quest.id === "calorie_target"
    ...
    <div
      ...
      <div className='flex items-start justify-between mb-3'>
        <div className='flex items-center gap-2'>
          ...
          <div>
            <p
              className={`font-ui font-semibold text-sm ${quest.completed ? "text-emerald" : "text-text"}`}>
              {quest.title}
            </p>
        ...
        {quest.completed ? (
          ...
          <span className='text-text-muted font-ui text-xs'>
            {Math.round(pct)}%
          </span>
        )}
      </div>
      ...
      <div className='flex items-center justify-between'>
        ...
        <div className='flex gap-1.5 flex-wrap'>
          {quest.reward?.xp && (
            <ResourceChip type='xp' amount={quest.reward.xp} size='sm' />
          )}
          {quest.reward?.iron && (
            <ResourceChip type='iron' amount={quest.reward.iron} size='sm' />
          )}
          {quest.reward?.gold && (
            <ResourceChip type='gold' amount={quest.reward.gold} size='sm' />
          )}
          {quest.reward?.energy && (
            <ResourceChip
              type='energy'
              amount={quest.reward.energy}
              size='sm'
            />
          )}
        </div>
...
function SectionHeader({title, sub}) {
  return (
    <div className='flex items-center justify-between mb-3'>
      ...
      {sub && (
        <p className='text-xs text-text-muted font-ui text-right'>{sub}</p>
      )}
    </div>
  );
}
