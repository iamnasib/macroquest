...
export default function AppLayout({ children }) {
  ...
  return (
    <div className="min-h-screen flex bg-void">
      {/* ── Mobile overlay ── */}
      {sidebarOpen && (
        <div className="fixed inset-0 bg-void/80 z-30 lg:hidden" onClick={() => setSidebarOpen(false)} />
      )}
          ...
          <div className="p-3 border-b border-border">
            <div className="bg-deep rounded-lg p-3 pixel-border">
              ...
              <div className="flex justify-between mt-1">
                <span className="text-xs text-text-muted font-ui">{levelData.currentXP} XP</span>
                <span className="text-xs text-text-muted font-ui">{levelData.xpNeeded} XP</span>
              </div>
            </div>
          </div>
        )}
